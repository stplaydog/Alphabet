...
a2 = sigmoid(z2);   %(25x5000) one by one
...
addOn = (lambda/(2*m))*(sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));
...
del2 = (del3*Theta2).*sigmoidGradient(zz2);
del2 = del2(:,2:end);

delta1 = delta1 + del2'*X;
