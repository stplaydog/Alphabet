...
\subsection*{Easy}
\begin{enumerate}
\item First bad version – bin search on array (recursive)
\item Search insert position
\item Search for a range – range of a duplication, search lower bound of self and +1.
\item Find peak element
\item Search in Rotated Sorted Array
\item Search in Rotated Sorted Array II (duplicate)
\item H-index II – already sorted
\item Search min in rotated sorted array
\item Search min in rotated sorted array II
\end{enumerate}
...
\item Search a 2D matrix II columns are also sorted
\end{enumerate}

\subsection*{Hard}
\begin{enumerate}
\item {\color{red} Median of two sorted arrays}
...
\item Flattern BT to linked list: 
\item Serialize and deserialize BT:
\item Verify Preorder Serialization of a Binary Tree:
\item Convert sorted list to BST:
\item Conver sorted array to BST:
\item Same tree:
\item Symmetric tree:
\end{enumerate}
